...
type AccessRule struct {
	...
	// Allowed path.
	// Can be an exact string or a string with a wildcard
	// e.g. :
...
type CkValidationState struct {
	...
	// Current status, should always be "pendingValidation"
	State string `json:"state"`
	// URL to redirect user in order to validate the ck
...
func (ck *CkRequest) Do() (*CkValidationState, error) {
	...
	if result.StatusCode != http.StatusOK {
		apiError := &APIError{ErrorCode: result.StatusCode}
